...
d = drawable(canvas=c, dimensions=dimension, coord=[50,50],
            xrange=[-0.6, t.getmax('rownumber')+0.6], yrange=[0.2, 0.4])
...
axis(drawable=d, style=axisstyle, ticstyle=axisticstyle,
     ...
     #     xtitle='Epoch',
         ...
         linewidth=lineWidth,
         xaxisposition=0.2, yauto=['','',0.05], xlabelrotate=0,
         ymanual=[['0.20', 0.2], ['0.25', 0.25], ['0.30', 0.3], ['0.35', 0.35], ['0.40', 0.4]],
         xlabelshift=[0, -2],
...
axis(drawable=d2, style='y', ticstyle=axisticstyle,
     ...
     #     xtitle='Similarity Threshold',
         ytitle='Accuracy (%)', doylabels=True, ytitleshift=[172,0], 
         ylabelshift=[160,0],
         # linewidth=0.8,
         linewidth=lineWidth,
         xaxisposition=0, yauto=['','',5], xlabelrotate=0,
     #     xlabelshift=[0, -20],
         xlabelfontsize=xlabelTextSize,
         ylabelfontsize=ylabelTextSize,
         xtitlesize=xtitleTextSize,
...
series_list = ['ttft']
fillcolors    = ['lightcoral', 'lightblue',  'lightgrey']
legend_names = ['TTFT', 'Accuracy']
...
for i in range(len(series_list)):
     ...
     p.points(drawable=d2, table=t2, xfield='rownumber', yfield=series_list[i], shift=[0, 0], size=4.5, legend=L, legendtext = legend_names[1], style=stylelst[i], fill=True, fillcolor=pointscolor[i],
...
L.draw(canvas=c, coord=[d.left()+5, d.top()+10], skipnext=1, 
     ...
     width=10, height=10, order=[0, 1, 2, 3])
