...
\begin{figure}
	...
	\includegraphics[width=3.3in, height=1.2in]{iotime.pdf}
	 \vspace{-0.1in}
	\caption{\zrdnew{Prefix KV loading time on the RTE dataset.}}
	\label{fig:iotime}
	\vspace{-0.1in}
\end{figure}
...
As ReComp and AS-like share the same accuracy, and AS+H2O+LRU and AS+H2O+LFU also show identical accuracy, we present only the accuracy of ReComp, AS+H2O+LRU, IMP, and \pname{} for simplicity.
It shows that \pname{} has a negligible impact on accuracy across these datasets
and models, achieving accuracy drop less than \zrdnew{x\%} compared to ReComp or AS+H2O+LRU. In
...
\fvc{
Figure~\ref{fig:overall_ttft} shows the average TTFT per request for different systems. \pname{} outperforms alternatives, with a \zrdnew{x}$\times$ to \zrdnew{x}$\times$ improvement over leading solutions, due to a \zrdnew{x}$\times$ to \zrdnew{x}$\times$ reduction in I/O time for loading prefix KVs into GPU memory (Figure~\ref{fig:iotime}). Other systems have longer I/O times, sometimes exceeding ReComp's TTFT. Besides, \pname{}'s performance gains vary across datasets and models due to different prefix KV sizes and computational demands. Notably, OPT-30B has a shorter TTFT than OPT-13B, as it uses shorter prefixes to avoid GPU memory overflow.
}
