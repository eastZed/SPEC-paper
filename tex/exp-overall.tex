...
\zrdnew{It shows that \pname{} has a negligible impact on accuracy across these datasets
...
\fvc{
Figure~\ref{fig:overall_ttft} shows the average TTFT per request for different systems. \pname{} outperforms alternatives, \zrdnew{with a 1.16$\times$ to 1.75$\times$ improvement over leading solutions, due to a 85.7$\%$ to 96.6$\%$ reduction in I/O time for loading prefix KVs into GPU memory on the inference critical path (Figure~\ref{fig:iotime})}. Other systems have longer I/O times, sometimes exceeding ReComp's TTFT. Besides, \pname{}'s performance gains vary across datasets and models due to different prefix KV sizes and computational demands. Notably, OPT-30B has a shorter TTFT than OPT-13B, as it uses shorter prefixes to avoid GPU memory overflow.
}
