...
Figure~\ref{fig:woandw-techa} illustrates the process of completing a transformer layer with and without this technique. Assume the prefix contains 4 tokens, with only one being important. Each transformer layer has 32 heads and all the prefix KVs of each head are stored on disks. The number of probe heads is set to three. 
Without it, all 32 heads must load 128 \textit{key} vectors and later 32 \(k,v\) vectors, totaling 160 vectors. 
With \techa{}, only 12 \textit{key} vectors from three probe heads are first loaded. The GPU computes attention and measures probe-head similarity. 
If the similarity exceeds the threshold, only one token index deemed most important by all probe heads \(\{x\}\) is used, and 64 \(k,v\) vectors are loaded and just 76 vectors in total. 
...
\begin{figure}
	\centering
	\includegraphics[width=3.4in]{simiload-ttft.pdf}
	\caption{\cp{
			Comparison of TTFTs with various approaches. (a) TTFTs without \techa{}. (b) TTFTs with \techa{}. (c) TTFTs with prefetching.
			Assume the LLM model consists
			of three transformer layers. The numbers inside the rectangles
			represent the layer index.
			The black dashed arrows in (b) indicate the dependency among operations.}}
	\label{fig:simiload-ttft}
\end{figure}


Figure~\ref{fig:simiload-ttft} compares the timelines for completing three transformer layers with and without \techa{}. 
Without \techa{} (Figure~\ref{fig:simiload-ttft}(a)), loading all keys causes significant GPU idle time, leading to a prolonged inference process. 
With \techa{} enabled (Figure~\ref{fig:simiload-ttft}(b)), when the similarity among probe heads exceeds the threshold, only the keys from the selected probe heads (in red) are first loaded to identify important tokens. 
After obtaining the important token positions, the system selectively loads only the corresponding keys and values instead of all tokens, effectively reducing I/O overhead. 
This selective loading also shortens the time required for generating important token index sets (in green), resulting in a shorter overall TTFT.

%However, as indicated by the black dashed arrows, dependencies among key loading, token selection, and value loading still limit the degree of overlap between computation and I/Os, resulting in partial idle periods on the GPU stream.
...
%}
To make the algorithm practical, we determine both the number of probe heads and the similarity threshold. Using a single probe head may introduce bias, while two heads can produce inconsistent results. Empirically, adopting three probe heads achieves a good balance between accuracy and efficiency. Adding more heads provides negligible accuracy improvement but increases key-loading latency and TTFT. Since attention heads exhibit high similarity, we simply select the first three heads in each transformer layer for efficiency. 
The similarity threshold also plays a critical role. A high threshold may fail to reduce KV loading because the measured similarity among probe heads often falls below it, whereas a low threshold may bias the selection of important tokens and degrade accuracy. As shown in Figure~\ref{fig:thred} on the PIQA~\cite{lmeval} dataset, each transformer layer selects the top 25\% most critical prefix KVs. Reducing the threshold from 1 to 0 decreases the number of loaded keys by 4$\times$, but also reduces accuracy from 79.1\% to 77.5\%. Similar trends are observed across other datasets.
