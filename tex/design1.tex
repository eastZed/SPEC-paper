...
\begin{figure}
	...
	\caption{\cp{
			...
			The LLM consists of three transformer layers, where the numbers in the rectangles denote layer indices. 
			In (b), the black dashed arrows indicate the dependency among the three operations, which prevents overlapping and causes idle periods on the GPU stream. 
			Prefetching in (c) alleviates this issue by overlapping KV loading with computation, further reducing TTFT.}}
	\label{fig:simiload-ttft}
\end{figure}


\cp{Figure~\ref{fig:simiload-ttft} compares the timelines for completing three transformer layers with and without \techa{}. 
...
%However, as indicated by the black dashed arrows, dependencies among key loading, token selection, and value loading still limit the degree of overlap between computation and I/O, resulting in partial idle periods on the GPU stream.
...
%}
\zrd{To make the algorithm practical, we determine both the number of probe heads and the similarity threshold. Using a single probe head may introduce bias, while two heads can produce inconsistent results. Empirically, adopting three probe heads achieves a good balance between accuracy and efficiency. Adding more heads provides negligible accuracy improvement but increases key-loading latency and TTFT. Since attention heads exhibit high similarity, we simply select the first three heads in each transformer layer for efficiency. 
