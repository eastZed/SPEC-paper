...
\zrdnew{Upon further investigation of the distribution of important token indices, we discovered that important token indices also exhibit strong consistency across adjacent layers. }
...
\zrdnew{Based on observation III, We propose the \technew{}mechanism. During the execution of the current layer's transformer computation, we perform speculative prefetching of important KVs for the next layer. This prefetching leverages the similarity of important token index sets across layers to improve the accuracy of speculative prefetching. Once the important token index set for the next layer is identified, any important tokens that were not pre-fetched are then loaded into memory.}