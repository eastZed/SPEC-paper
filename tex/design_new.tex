...
In contrast, Figure~\ref{fig:simiload-ttft}(c) shows that using Layer~0’s importance information to prefetch Layer~1’s KVs during computation effectively hides I/O latency and reduces idle time.}

Two issues must be addressed for efficient prefetching. 
First, since approximating the next layer’s important token distribution using that of the current layer is inherently imprecise, prefetching becomes speculative, making it crucial to improve its accuracy. 
Second, the I/O operations for prefetching must be well overlapped with computation. 
If computation takes longer than prefetching, the I/O bandwidth remains underutilized; conversely, if prefetching exceeds computation time, speculative I/O appears on the critical path, and any mispredicted prefetches can introduce additional latency. 

To tackle the first issue, based on \textit{Observation~III}, we prioritize prefetching the KVs corresponding to tokens that have higher importance in the current layer, as these tokens are more likely to remain important at the same indices in the subsequent layer. 
...
For computation, we observe that during the prefill stage of inference, the computational workload across transformer layers is approximately uniform. 
...
For I/O estimation, key and value vectors are stored on a per-chunk basis, where each chunk contains multiple token vectors (e.g., \texttt{chunksize}=64). 
Given a set of target token IDs, we first map each token to its corresponding chunk and identify the storage medium. 
Chunks on SSD are first \texttt{mmap}-loaded into CPU memory, after which the required K/V vectors are indexed and transferred to GPU memory via PCIe. 
For chunks already in CPU memory, the K/V vectors are directly fetched and transferred over PCIe, while GPU-resident chunks are accessed directly. 
Our profiling shows that the latency of fetching a chunk’s K/Vs remains stable within each storage tier and follows a near-normal distribution with low variance, indicating predictable performance. 
We thus use the average per-chunk latency for each medium as a profiling result and sequentially fill the prefetching window by loading chunks until the estimated budget is fully utilized. 
This approach effectively balances computation and I/O, ensuring that speculative prefetching remains hidden behind ongoing GPU execution.
