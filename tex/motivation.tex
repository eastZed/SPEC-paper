\section{Design Challenges of \pname{}}
\label{motiv}
\begin{figure}
	\centering
...
\fvc{Existing methods identify important KVs by loading all prefix keys (K) into GPU memory~\cite{h2o-nips23, infinigen-osdi24, flexgen-icml23, scissorhands-nips23}, 
...
During inference, prefetching can be performed by loading the KVs required for the next transformer layer in advance, while the current layer is being computed~\cite{attentionstore-atc24}. This technique allows I/O operations and computation to overlap, thereby hiding data loading latency and improving pipeline efficiency. However, in \pname{}, only the important KVs are loaded to reduce unnecessary I/Os. These important KVs for each layer can only be identified after computing that layer's attention weights. This creates a fundamental constraint: when processing \textit{layer k}, the system does not yet know which KVs will be critical for \textit{layer k+1}, since \textit{layer k+1}'s attention computation has not occurred. As a result,  approaches cannot prefetch the next layer's required KVs in advance, leaving I/O bandwidth idle during computation and forcing expensive synchronous loads.}

\zrdnew{A direct method is to prefetch KV pairs for the next layer based solely on their historical importance, independent of the current query. Under this history-based method, KV pairs that were frequently important in the past are always prioritized.
%Concretely, for each KV pair in a prefix, we maintain a historical importance score that aggregates how often and how strongly it has contributed to past model outputs (e.g., via accumulated selection frequency). At inference time, we rank KV pairs in the prefix by these scores and prefetch the top-M candidates to GPU for the next layer, regardless of the current query. 
However, KV importance is highly query-dependent: a token that is critical for one query may be irrelevant for another, making a single global importance score an unreliable proxy for per-request importance. Moreover, obtaining reliable per-KV statistics requires many requests and assumes a relatively stable workload. In \S\ref{exp:prefetch}, we empirically show that this history-based method underperforms our method in prefetch precision and critical-path I/O reduction under the same prefetch budget.}
