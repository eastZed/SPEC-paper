...
During inference, prefetching can be performed by loading the KV pairs required for the next transformer layer in advance, while the current layer is being computed. This technique allows I/O operations and computation to overlap, thereby hiding data loading latency and improving pipeline efficiency. However, in \pname{}, only the important KVs are loaded to reduce unnecessary I/O. These important KVs are determined based on the attention weights of the current layer, meaning that the important tokens for the next layer remain unknown until the current computation finishes. Consequently, the system cannot prefetch the next layer’s required KVs in advance, leaving I/O bandwidth underutilized.}
