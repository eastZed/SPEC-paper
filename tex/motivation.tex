...
\zrdnew{A direct method is to prefetch KV pairs for the next layer based solely on their historical importance, independent of the current query. Under this naive history-driven strategy, KV pairs that were frequently important in the past are always prioritized.
Concretely, for each KV pair in a prefix, we maintain a historical importance score that aggregates how often and how strongly it has contributed to past model outputs (e.g., via accumulated selection frequency). At inference time, we rank KV pairs in the prefix by these scores and prefetch the top-M candidates to GPU for the next layer, regardless of the current query. However, KV importance is highly query-dependent: a token that is critical for one query may be irrelevant for another, making a single global importance score an unreliable proxy for per-request importance. Moreover, obtaining reliable per-KV statistics requires many requests and assumes a relatively stable workload. In \S\ref{exp:prefetch}, we empirically show that this history-based prefetcher underperforms our layer-aware importance predictor in prefetch precision and critical-path I/O reduction under the same prefetch budget.}
