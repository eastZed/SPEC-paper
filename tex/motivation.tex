...
\zrdnew{A direct method is to prefetch KV pairs for the next layer based solely on their historical importance, independent of the current query. In this naive history-based baseline, we always prioritize KV pairs that have been useful most often in the past.
Concretely, for each KV pair in a prefix, we maintain a historical importance score that aggregates how often and how strongly it has contributed to past model outputs (e.g., via accumulated selection frequency). At inference time, we rank KV pairs in the prefix by these scores and prefetch the top-
M candidates to GPU for the next layer, regardless of the current query. However, KV importance is highly query-dependent: the same prefix token can be crucial for one query and irrelevant for another, so a single global score is often a poor proxy for per-request importance. Moreover, obtaining reliable per-KV statistics requires many requests and assumes a relatively stable workload. In §X, we empirically show that this history-based prefetcher underperforms our layer-aware importance predictor in prefetch precision and critical-path I/O reduction under the same prefetch budget.}
