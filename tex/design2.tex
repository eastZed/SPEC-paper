...
\zrd{We present the \techba{} method to address the unnecessary loading of unimportant KVs during important KV retrieval. By periodically reorganizing and repacking important KVs into denser chunks, this approach optimizes read efficiency and reduces bandwidth waste.
	Scheduled at regular intervals (e.g., every 10 minutes), this process is based on the average token importance and operates asynchronously to avoid disrupting the main I/O flow.}
...
%}
\zrd{
	To illustrate, consider a prefix $[t_0, t_1, t_2, t_3, t_4, t_5]$ where the existing system stores keys for two adjacent tokens in one chunk, and each chunk mixes one high-importance and one low-importance key. Assume the importance ranking among the important keys is $t_0 \succ t_4 \succ t_3$.
	When loading,
	without reordering (Figure~\ref{fig:reordering}\,(a)), all chunks must be loaded to retrieve the important keys $\{t_0, t_3, t_4\}$, wasting read bandwidth and cache space on low-importance keys. With \techba{} as in IMPRESS (Figure~\ref{fig:reordering}\,(b)), tokens are reordered by importance, and keys of adjacent reordered tokens (e.g., $[t_0, t_3]$, $[t_1, t_2]$, $[t_4, t_5]$) are packed into single chunks. This enables loading only two chunks to access all important keys, thereby reducing disk traffic. In HyperInfer (Figure~\ref{fig:reordering}\,(c)), keys of adjacent reordered tokens are instead packed as $[t_0, t_4]$, $[t_2, t_3]$, and $[t_1, t_5]$ according to the importance ranking, which also requires loading only two chunks.
	When prefetching,
	Assume the time budget allows loading only one SSD chunk. Without reordering, as well as with IMPRESS, prefetching can cover only one potentially important key, $\{t_0\}$, whereas HyperInfer can prefetch two, $\{t_0, t_4\}$.
}
