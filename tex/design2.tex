...
\zrd{
	...
	For example, if the time budget allows prefetching only one SSD chunk, the ideal prefetching order based on importance would retrieve $t_0$ and  $t_4$. 
	However, under the data layout of IMPRESS, $t_0$ and $t_4$ are stored in different chunks, while $t_0$ and $t_3$ share one. 
	As a result, only $\{t_0, t_3\}$ can be prefetched, leaving one highly important key $t_4$ unavailable within the time limit. 
