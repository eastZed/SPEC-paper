...
\zrdnew{The core idea is to load only the KVs of important prefix tokens, minimizing I/Os from slower disks, while prefetching future potentially critical KVs to overlap I/Os with computation, further reducing latency. However, building such an efficient system involves three significant challenges.}
...
% them using caches, aiming to enhance disk I/Os and PCIe transfer efficiency. However, since our system selectively reads only the important KVs, directly applying these existing storage and cache management methods is suboptimal for two main reasons. 
...
(e.g., chunks), to optimize disk I/Os and PCIe transfer bandwidths.
...
\zrdnew{To address the second challenge, we leverage the observation that the sets of important token incices are highly similar across neighboring layers. Building on this property, we design the \technew{} mechanism, which uses the important tokens identified in the current layer to approximate those in the subsequent layer. This enables the system to prefetch the next layer’s KVs while the current layer is being computed, effectively overlapping I/Os with computation and further reducing TTFT.
}
...
\item \cp{We propose \technew{} mechanism to prefetch important KVs, effectively overlapping I/Os with computation.}
\item We introduce a similarity-guided important token identification method for efficiently locating important KVs, and complement it with \techb{}, which incorporates \techba{} and a new \techbb{} policy to further minimize data movement from slower storage media.


\item We implement \pname{} and our evaluation demonstrates that it reduces TTFT by up to \zrdnew{1.75}$\times$ compared to the state-of-the-art prefix KV storage systems while maintaining comparable inference accuracy.
\end{itemize}
